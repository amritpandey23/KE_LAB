...
public class Assignment_7 {
    ...
    static void generateAllConditionalFPTreeBranchTraversals(TreeNode root, String currentPattern,
            HashSet<String> allPossiblePatters) {
        if (root == null)
            return;
        if (!root.val.equals("NULL"))
    ...
    static void preorder_pattern_record(TreeNode root, String currentPattern, HashMap<String, Integer> possiblePattern,
            ArrayList<TreeNode> headerTableEntries, Node currNode) {
        if (root == null) {
            return;
        } else if (currentHeaderTableIndex >= headerTableEntries.size()) {
            return;
        } else if (headerTableEntries.get(currentHeaderTableIndex).val.equals(root.val)
                && headerTableEntries.get(currentHeaderTableIndex).cnt == root.cnt) {
            // System.out.print(headerTableEntries.get(currentHeaderTableIndex).val + " -- "
            // + root.val + " : " + currentPattern);
            // System.out.println();
